...
const possibleBipartition = (n, dislikes) => {
    ...
    let adj = [...Array(n+1)].map(x => [])
    for (let [a, b] of dislikes) {
        adj[a].push(b)
        adj[b].push(a)
    }
    // console.log(adj)
    let colours = Array(n+1)
    let stack = []
    let pushed = new Set()
    for (let i = 1; i <= n; i++) {
        ...
        while (stack.length) {
            let cur = stack.pop()
            for (let disliked of adj[cur]) {
                if (colours[cur] === colours[disliked]) {return false}
                if (colours[disliked] === undefined) {colours[disliked] = 1 - colours[cur]}
                if (!pushed.has(disliked)) {
                    stack.push(disliked)
                    pushed.add(disliked)
                }
            }
...
tests.forEach((t, i) => {
    console.log('test:', i, possibleBipartition(t.n, t.dislikes), 'should be', t.out)
})