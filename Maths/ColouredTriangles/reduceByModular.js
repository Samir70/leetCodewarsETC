...
const { pairs, colourDigits, colourLetters } = require('./exportFrom');
...
const reduceByModular = (row) => {
    ...
    switch (len) {
        ...
        // for 7+, len should be a goodLength
        default: {
            return sumMod3(digits, whichToAdd(len))
        }
    }
}
...
const triangle = (row) => {
    ...
    if (bestLength === len) { return reduceByModular(row) }
    var nextRow = '';
    for (var i = 0; i+bestLength<=len; i++) {
        var segment = row.slice(i, i+bestLength);
        nextRow += reduceByModular(segment);
    }
